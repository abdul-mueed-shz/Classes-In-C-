...
public:
	...
	ulist(int s)//parametric constructor
	{
		size = s;
		arra = new int[size];
	...
	ulist(const ulist &carra)//copy constructor
	{
		size = carra.size;
	...
	friend ostream &operator<<(ostream &out, ulist &outarray)//for printing
	{
	...
	void operator=(ulist &carra)//for assigning
	{
		size = carra.size;
		currentindex = carra.currentindex;
		arra = new int[size];
